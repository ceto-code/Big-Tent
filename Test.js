...
contract("BigTent", accounts => {
  it("Buy 10 tickets with cost of each ticket as 1.1ceto , GPP = 9.9ceto - no partner", async () => {
    ...
    var initialGuaranteedPrizePool =
      await contract.initialGuaranteedPrizePool.call();
    console.log(
  ...
  it("Results for case4", async () => {
    ...
    var partner_tokens = await ceto.myTokens().call({ from: accounts[1] });
  ...
  it("Withdraw test", async () => {
    ...
    var tokens_ = await ceto
      .calculateTokensReceived(561)
      .call({ from: accounts[0] });
    var Withdraw = await contract.withdrawRebate(561, {
      from: accounts[0],
      feeLimit: 1e8,
    });
    console.log("Withdraw : ", Withdraw);
    var expected_tokens_gained = Math.round(
      parseInt(tokens_.toString()) - parseInt(tokens_.toString()) / 10
    );
    ...
    var actual_tokens_gained =
      parseInt(tokens.toString()) - parseInt(tokens1.toString());
  ...
  it("Buy using Tron", async () => {
    ...
    // var buy_ = await ceto.methods
    //   .buy(accounts[0])
    //   .send({ from: accounts[1], value: amount});
    // console.log(buy_, "buy_");
    var tokens = await ceto.methods.myTokens().call({ from: accounts[0] });
    console.log("tokens", tokens.toString(), accounts[0]);
    ...
    await axios(config)
      .then(function (response) {
        ...
        cetoPrice =
          parseFloat(JSON.stringify(response.data.data)).toFixed(6) * 1e6;
